...
# for project_dir in "${kin_parents_project_dirs[@]}";do echo project_dir:$project_dir;done
do_find_out_kin() {
  level_counter=0
  kin_parents_project_dirs=()

  if [ "${USE_KIN_HIERARCHY:-1}" -eq 1 ]; then
    if [ -f "$PRODUCT_DIR/cnf/$PRODUCT.kin.yaml" ]; then
      cp -v "$PRODUCT_DIR/cnf/$PRODUCT.kin.yaml" "$PRODUCT_DIR/cnf/$PRODUCT.tmp.kin.yaml"
      perl -pi -e 's|%BASE_DIR%|'$BASE_DIR'|g' "$PRODUCT_DIR/cnf/$PRODUCT.tmp.kin.yaml"

      yq e '.. | select(has("project_name")) | .project_name' \
        "$PRODUCT_DIR/cnf/$PRODUCT.tmp.kin.yaml" > "$PRODUCT_DIR/dat/log/tmp_project_names.txt"

      while read -r project_name; do
        level_counter=$((level_counter + 1))
        while read -r project_dir; do
          kin_parents_project_dirs+=("$project_dir")
        done < <(yq e ".. | select((has(\"project_name\") and has(\"project_dir\")) \
                          and (path | length == ($level_counter - 1))) | .project_dir" "$PRODUCT_DIR/cnf/$PRODUCT.tmp.kin.yaml")

        if [ "$project_name" == "$PRODUCT" ]; then
          echo "Level number: $level_counter"
          break
        fi
      done < "$PRODUCT_DIR/dat/log/tmp_project_names.txt"

      rm "$PRODUCT_DIR/dat/log/tmp_project_names.txt"
    fi
    kin_parents_project_dirs+=("$PRODUCT_DIR")
    test -f "$PRODUCT_DIR/cnf/$PRODUCT.tmp.kin.yaml" && rm -v "$PRODUCT_DIR/cnf/$PRODUCT.tmp.kin.yaml"
  fi
}
...
do_run_actions(){
  ...
  run_funcs=''
  ac=0
  array_length=${#kin_parents_project_dirs[@]}
  actions="$(echo -e "${actions}"|sed -e 's/^[[:space:]]*//')"  #or how-to trim leading space

  # Use a while loop to cycle through the elements of the array
  while [ $ac -lt $array_length ]; do
    project_dir=${kin_parents_project_dirs[$ac]}

    while read -d ' ' arg_action ; do
        while read -r fnc_file ; do
          #debug echo fnc_file:$fnc_file
          while read -r fnc_name ; do
              #debug echo fnc_name:$fnc_name
              action_name=`echo $(basename $fnc_file)|sed -e 's/.func.sh//g'`
              action_name=`echo do_$action_name|sed -e 's/-/_/g'`
              #debug echo action_name: $action_name
        ...
        done < <(find "$project_dir/lib/bash/funcs" "$project_dir/src/bash/run/"  -type f -name '*.func.sh'|sort)
    done < <(echo "$actions")
    ((ac++))
  done
  ...
  run_funcs=$(echo "$run_funcs" | sort | uniq)
...
do_log(){
  ...
  type_of_msg=$(printf '%-6s' "$type_of_msg")
  msg="$(echo $*|cut -d" " -f2-)"
...
do_load_functions(){
  ...
  while [ $ac -lt $array_length ]; do
    ...
    cd $project_dir
      ...
      while read -r f; do source $f; done < <(ls -1 $project_dir/lib/bash/funcs/*.func.sh)
      while read -r f; do source $f; done < <(ls -1 $project_dir/src/bash/run/*.func.sh)
    ((ac++))
  done
...
do_resolve_os(){
   if [[ $(uname -s) == *"Linux"* ]]; then
       ...
       if [[ $distro == 'ubuntu' ]] || [[ $distro == "pop" ]] || [[ $distro == 'debian' ]]; then
         export OS='ubuntu'
